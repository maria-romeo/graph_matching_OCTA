...
from sitk_files.registration_utilities import registration_errors
...
def metric_and_reference_plot_values(registration_method, fixed_points, moving_points):
    ...
    current_transform = sitk.Transform(
        registration_method.GetInitialTransform()
    )
    print(current_transform.GetDimension())
    initial = registration_method.GetMovingInitialTransform()
    print(initial)
    print(initial.GetDimension())
