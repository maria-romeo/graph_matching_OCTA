...
sweep_configuration = {
    ...
    "parameters": {
        ...
        "dataset": {"values": ['random_25_drop', 'random_50_drop', 'random_75_drop', 'random_100_drop']}, #, "DCP"
        ...
        "hungarian": {"values": [True]}, # True, False [4, 4, 5, 5, 7],
    },
}
...
def main():
    ...
    if not os.path.exists(f'GM_results_{wandb.config.dataset}/baseline/baseline_matrix_results'):
        os.makedirs(f'GM_results_{wandb.config.dataset}/baseline/baseline_matrix_results')
    x_pred_dir = f'GM_results_{wandb.config.dataset}/baseline/baseline_matrix_results'

    # Iterate over DataLoader
    for batch_idx, (index, gt) in enumerate(dataloader):
    ...
    with open(f'GM_results_{wandb.config.dataset}/baseline/baseline_config_info.txt', 'a') as config_file:
    ...
    results_df.to_csv(f'GM_results_{wandb.config.dataset}/baseline/baseline_evaluation_metrics.csv', index=False)

    # Save the evaluation metrics
    with open(f'GM_results_{wandb.config.dataset}/baseline/baseline_avge_evaluation_metrics.txt', 'a') as metrics_file:
